...
#python3 stream_save_track_unwarped2.py --key GroundSystem.private.key --cert GroundSystem.cert.pem --client_id groundSysVideo --topic groundsys/video --endpoint aicgt013q5xy-ats.iot.us-east-1.amazonaws.com --ca_file root-CA.crt --count 0
 
#*** ATTEMPT FOR STREAMING A HIGHER QUALITY UN_FISHEYED STREAM 1024p
...
#***FROM ATTEMPT TO HAVE CODE SELF-OPEN JITSI MEETING, DOES NOT WORK
...
# main computation function, opens video stream, un-fisheyes frame using
# maps from callibration, 
# creates and reroutes to virtual port, 
# saves to recording if in a test
def stream_and_save(video_path, device):
    global rec_start, rec_count, released, init_rec
   
    mapx = np.load("calibration/calibration_mapx.npy")
    mapy = np.load("calibration/calibration_mapy.npy")
    roi = np.load("calibration/calibration_roi.npy")
    ...
    width = 1280#int(video.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = 720#int(video.get(cv2.CAP_PROP_FRAME_HEIGHT))
    ...
    #***FROM ATTEMPT TO HAVE CODE SELF-OPEN JITSI MEETING, DOES NOT WORK
    # join call
    # referenced: https://www.askpython.com/python/examples/auto-join-zoom-meetings
    # for set up
    
        # service = Service(executable_path="~/.mozilla/firefox/")
        # driver = webdriver.Firefox(service=service)
        # #driver = webdriver.Firefox(executable_path="~/.mozilla/firefox/")
        # driver.get("https://meet.jit.si/")

        # meet_code = "helloRPCS"

        # element_box = driver.find_element_by_xpath("//input[@id='enter_room_field']")
        # element_box.send_keys(meet_code)

        # time.sleep(2)

        # btn = driver.find_element_by_xpath("//a[@id='enter_room_button']")
        # btn.click()
    ...
    # start frame streaming
        ...
        while True:
            ...
            resize_frame = cv2.resize(raw_frame,(1024,720))
            cam.send(raw_frame)
            ...
            # resize so frame after un-fisheye fits recording format
            # frame = cv2.resize(frame,(1280,720))

            # if rec_start == True:
            #     if init_rec == False:
            #         video_name = 'recordings/stream'+str(rec_count)+ '.mp4' #'.avi'
            #         print("video " + video_name)
            #         fourcc = cv2.VideoWriter_fourcc(*'MP4V')
            #         #rec_out = cv2.VideoWriter(video_name, fourcc, 20.0, (int(video.get(3)), int(video.get(4))))
            #         rec_out = cv2.VideoWriter(video_name, fourcc, 20.0, (1280, 720))
            #         init_rec = True
            #     rec_out.write(frame)
